package grgTakeOutProject;

import java.util.Scanner;

public class GrgTakeOut1 {
	 ...
	 public static void main(String[] args) {
	        ...
	        do {
	            ...
	            switch (mealType) {
	                ...
	                case "1":
	                	GrgTakeOut1.displayItalianFoodMenu();
	                    int[] feedItalianFood = {5, 7, 4};
	                    double[] priceItalian = {17.99, 15.99, 12.99};
	                    int opt = GrgTakeOut1.foodChoice(feedItalianFood, priceItalian);
	                    GrgTakeOut1.finalPrice(feedItalianFood, priceItalian, opt, numberOfPeople);
	                    isWrongMenuChoice = false;
	                    break;

	                case "2":
	                	GrgTakeOut1.displayChineseFoodMenu();
	                    int[] feedChineseFood = {7, 7, 5};
	                    double[] priceChinese = {18.99, 18.99, 10.99};
	                    opt = GrgTakeOut1.foodChoice(feedChineseFood, priceChinese);
	                    GrgTakeOut1.finalPrice(feedChineseFood, priceChinese, opt, numberOfPeople);
	                    isWrongMenuChoice = false;
	                    break;

	                case "3":
	                	GrgTakeOut1.displayAmericanFoodMenu();
	                    int[] feedAmericanFood = {8, 5, 10};
	                    double[] priceAmerican = {21.99, 22.99, 26.99};
	                    opt = GrgTakeOut1.foodChoice(feedAmericanFood, priceAmerican);
	                    GrgTakeOut1.finalPrice(feedAmericanFood, priceAmerican, opt, numberOfPeople);
	                    isWrongMenuChoice = false;
	                    break;
	                default:
	    ...
	    public static void displayAmericanFoodMenu() {
		    ...
		    public static void finalPrice(int[] feeds, double[] prices, int choice, int guestCount) {
		        int trays = GrgTakeOut1.determineTrays(guestCount, feeds[choice-1]);
		        double subTotal = GrgTakeOut1.getSubtotal(prices[choice - 1], trays);
		        double tax = GrgTakeOut1.getTax(subTotal, .07);
		        double tip = GrgTakeOut1.getTip(subTotal, .15);
		        double grandTotal = GrgTakeOut1.getGrandTotal(subTotal, tax, tip);
		        ...
		        System.out.printf("Price per person: $ %.2f\n", GrgTakeOut1.pricePerPerson(grandTotal, guestCount));
		        System.out.println("Left-over servings for the delivery person: " + GrgTakeOut1.determineLeftOvers(feeds[choice - 1], trays, guestCount));
